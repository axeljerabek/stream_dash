...
fn main() -> io::Result<()> {
    ...
    let mut state = AppState {
        version: "23.0-rs".to_string(), interval_ms: 1000, log_lines: 10, color_mode: 0,
    ...
    loop {
        ...
        draw_stacked_graph(&mut stdout, mid_x, 11, &state.cma_history, c_cma1, c_cma2)?;
        ...
        execute!(stdout, MoveTo(0, 17), SetForegroundColor(c_sec), Print("[HARDWARE & HEALTH]"), MoveTo(mid_x, 17), Print("[DMA-BUFFER DETAILS]"))?;
        ...
        execute!(stdout, MoveTo(0, 22), SetForegroundColor(c_main), Print(format!("Net: {:>5} kbps Out", net_speed)), ResetColor, Print(" | "), SetForegroundColor(c_main), Print(format!("Stream: {}", run_cmd("ps", &["-C", "rpicam-vid,ffmpeg", "-o", "comm="]))))?;
        execute!(stdout, MoveTo(0, 23), SetForegroundColor(Color::DarkGrey), Print("-".repeat(cols as usize)))?;

        let log_y = 24;
        execute!(stdout, MoveTo(0, log_y), SetForegroundColor(c_sec), Print(format!("[SYSTEMD LOG: stream.service ({} lines)]", state.log_lines)))?;
            ...
            if let Event::Key(key) = event::read()? {
                if key.kind == KeyEventKind::Press {
                    match key.code {
                        ...
                        KeyCode::Char('c') => { state.color_mode = (state.color_mode + 1) % 3; execute!(stdout, Clear(ClearType::All))?; },
